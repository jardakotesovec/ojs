...
- **Placement:** lib/pkp
- **Budget:** 6 tests
- **Absorbs:** lib/pkp/playwright/tests/publication-metadata-editing.spec.js — that spec now holds 3 tests backing 3 rows (N-test absorption rule): the title/abstract/keywords test (row 1 here), the wave-5 multilingual add-contributor test (row 2 here), and the original en-only add-contributor test, which stays owned by the contributors plan row 1. Both plans record this split. Also: lib/pkp/playwright/tests/section-editor-metadata.spec.js, lib/pkp/playwright/tests/author-edit-published.spec.js
...
| 2 | Editor adds a contributor via the Contributors panel | dbarnes | submission-in-review fixture | Add Contributor modal saves (multilingual name — en + fr_CA via the FormLocales toggle —, email, country, required contributor role); new contributor appears in the manager list and in the publication's authors via API (both name locales, country and role round-trip) | implemented (lib/pkp/playwright/tests/publication-metadata-editing.spec.js) |
