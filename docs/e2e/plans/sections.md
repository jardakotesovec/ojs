...
- **Scenario needs:** journal scenario endpoint (scratch journal, `users`); its `sections` array (abbrev/title/wordCount/identifyType/abstractsNotRequired/editorRestricted/sectionEditors) pre-seeds the multi-section state for row 6 so the UI only drives reorder/delete. Section CRUD itself goes through the grid UI — the surface under test. Row 6 seeds a per-test throwaway manager (`users[]` with `password`) so the delete-refusal error toasts can be asserted without racing the shared dbarnes notification queue, and uses the `playwright/pages/SectionsSettingsPage.js` POM (created for row 6; consolidates the grid helpers other specs had been duplicating). No gaps.
