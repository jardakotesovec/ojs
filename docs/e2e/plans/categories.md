...
- **Budget:** 5 tests
- **Absorbs:** lib/pkp/playwright/tests/categories.spec.js (4 tests → rows 1–4; sole owner — `submission-wizard-metadata` released its claim). Row 5 is split into the OJS tree as playwright/tests/categories-publication.spec.js (1 test) because its surface is OJS's publication "Publication Settings" (IssueEntry) panel.
