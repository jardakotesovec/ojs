...
- **Placement:** ojs
- **Budget:** 5 tests
- **Absorbs:** playwright/tests/native-xml-submission.spec.js (2 tests → rows 1 + 4; row 4 extends the same file rather than a new spec — the Tools-UI journey is the browser-path complement of row 1's raw-request flow, same seed shape); playwright/tests/native-xml-issue.spec.js (2 tests → rows 2–3); playwright/tests/pubmed-metadata.spec.js (2 tests → row 5)
...
| 4 | Manager drives the Native XML export through the Tools UI | dbarnes | scenario: submission-published (publicknowledge) | Tools → Import/Export → Native XML Plugin: the Export Articles tab's submissions-list-panel lists the seeded submission (search by hyphenless tag, exact-count 1); select → Export Articles (bounce-tab renders "The export completed successfully.") → Download Exported File fires a real browser `download` event (Content-Disposition: attachment) whose bytes are native XML containing the tagged title (covers the legacy tab/AjaxFormHandler UI path the API-driven rows bypass; note the panel is a Vue list panel, not the legacy grid the row title presumed) | implemented (playwright/tests/native-xml-submission.spec.js) |
