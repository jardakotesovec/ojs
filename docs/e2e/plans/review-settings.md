...
- **Placement:** lib/pkp
- **Budget:** 5 tests
- **Absorbs:** lib/pkp/playwright/tests/reviewer-recommendations.spec.js (5 tests → row 5; previously orphaned — no plan claimed it). **Refit note (done, wave 10):** the spec already carried one test per recommendation-settings concern (defaults/type metadata, custom-recommendation CRUD, active/inactive toggle, in-use lock, inactive option absent from the reviewer's completion form); the refit re-pointed its header at this plan (it still referenced the retired migration-doc row #6) and removed two shadowing inline `require`s — behavior-identical, no coverage change.
