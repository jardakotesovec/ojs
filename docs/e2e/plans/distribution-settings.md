...
- **Scenario needs:** scratch journal via `POST /api/v1/_test/scenarios/journal` (`publishingMode` passthrough available where a pre-set mode is needed); `submission-published` fixture with `journal` override (row 1 — note: the fixture hard-codes copyrightHolder/copyrightYear/licenseUrl metadata, which must be stripped from the spec so the publish backfill can apply the journal defaults; lib/pkp publication Repository.php:580,602 only fills EMPTY fields). Settings values are driven through the UI — the mutation is the behavior under test.
