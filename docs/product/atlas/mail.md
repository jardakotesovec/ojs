...
| MAIL-user-created | UserCreated | lib/pkp/classes/mail/mailables/UserCreated.php | template key: USER_REGISTER; sent when a MANAGER adds a user (UserDetailsForm), NOT on self-registration | user-management | registration-login references it (trigger is the manager add-user form; claimed by user-management 2026-07-05) |
| MAIL-user-role-assignment-invitation-notify | UserRoleAssignmentInvitationNotify | lib/pkp/classes/mail/mailables/UserRoleAssignmentInvitationNotify.php | template key: USER_ROLE_ASSIGNMENT_INVITATION | user-invitations | claimed by user-invitations 2026-07-05 (the "you've been invited to roles" email; its trigger is the invite-send). Reassigned from notifications. |
| MAIL-user-role-end-notify | UserRoleEndNotify | lib/pkp/classes/mail/mailables/UserRoleEndNotify.php | template key: USER_ROLE_END | user-management | fired by API-user-end-role (claimed by user-management 2026-07-05) |
| MAIL-user-role-masthead-update-notify | UserRoleMastheadUpdateNotify | lib/pkp/classes/mail/mailables/UserRoleMastheadUpdateNotify.php | template key: USER_ROLE_MASTHEAD_UPDATE | notifications | claimed by editorial-masthead 2026-07-05 (the "your masthead visibility has been updated" email; fired on a per-user masthead flip by API-user-masthead in user-management, no-op sends nothing, reviewer refused; live-verified delivery + no-op suppression). Reassigned from notifications. |
