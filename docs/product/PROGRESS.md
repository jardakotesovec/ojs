...
| 9 | Area 2 — Editorial workflow & peer review | `stage-participants` | M·8 | pending | pending | REBUILD EXPERIMENT (maintainer, 2026-07-22): wave-8 build scrapped (lib/pkp revert 9c81807da7, spec removed) to validate TEMPLATE rule 5 end-to-end; ledger rows 229–234 + all wave-8 amendments/exposure re-validations STAND (a probe contradicting one is a finding, not a reconcile); THIS FEATURE: attempt a MONOLITHIC test-author FIRST even though the spec is permission-matrix-centric — the experiment measures whether rule-5 phrasing removes the flip trigger; on flip, standard discard+respawn, then the split protocol as usual |
