...
| 8 | Area 2 — Editorial workflow & peer review | `editorial-decisions` | H·13 | pending | pending | from-scratch rebuild (maintainer, 2026-07-21): wave-7 spec+tests scrapped after static review of the opus-tail authored suite (lib/pkp revert ad338c097b, spec removed 6cb6473100); ledger rows 214–219 + row-10 addendum STAND — a probe contradicting one is a finding, not a reconcile; THIS FEATURE ONLY: authoring output must end fable-clean — a flipped authoring chunk is finished, logged, DISCARDED and respawned fresh (max 2 respawns, then park); verification/probes stay under normal policy |
