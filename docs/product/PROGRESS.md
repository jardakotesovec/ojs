...
| 9 | Area 2 — Editorial workflow & peer review | `stage-participants` | M·8 | verified | pending | TESTS-ONLY REBUILD EXPERIMENT (maintainer, 2026-07-22): SPEC verified & KEPT — the rule-5-swept version (density 10→3, commit f036c96559); wave-8 tests + POM extensions reverted in lib/pkp (9c81807da7) to re-author against the swept spec; ledger rows 229–234 + all wave-8 amendments STAND; THIS FEATURE: run RUNBOOK steps 5–9 only, with a MONOLITHIC test-author FIRST despite the permission-matrix spec — the experiment tests whether rule-5 phrasing alone removes the flip trigger (wave-8 control: 2 monolithic authors flipped on the unswept wording); on flip, standard discard+respawn, then split protocol |
