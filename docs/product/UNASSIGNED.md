...
| GRID-lib-pkp-grid-files-submission-editor-submission-details-files-grid-handler | Concrete class extends `FileListGridHandler` but has no component-path reference, no subclass, and no `new` instantiation anywhere; its sibling `AuthorSubmissionDetailsFilesGridHandler` (author-facing) is live, this editor-facing one is not | VUE-file-manager | 2026-07-02 |
