...
gitignored `.reports/opus-eval/STATE.md` (2026-07-25; that file kept the eval's
execution history only and has since been deleted). Delete this file when the queue is empty. The standing
...
here when done. Area 2 (row 12 onward) resumes after items 1–2.
...
2. **Re-scope + encoding session** (one session): GLOSSARY seed input — the
   ...
   sunset clause).
3. **Process deliberation list (maintainer still thinking; do not land):**
...
4. **UNASSIGNED grooming pass (unscheduled — maintainer to slot, e.g. at a
