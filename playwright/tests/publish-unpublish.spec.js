...
/**
 * Publication publish flow — row 1 in
 * docs/e2e/plans/publication-publish-flow.md.
 *
 * Moved from lib/pkp/playwright/tests/publish-unpublish.spec.js (wave 6):
 * publish/unpublish targets the OJS issue-based article front end, so per
 * the placement rule it belongs in the OJS tree. Original port of the
 * publish / unpublish slice of AmwandengaSubmission.cy.js tests 6 and 7:
...
async function expectArticleRendered({
	...
	// Empty storageState keeps this context anonymous even if a future
	// edit adds test.use({user}) — browser.newContext() inherits the
	// test's storageState option otherwise.
